(function () {
            ...
            installingWorker.onstatechange = (e) => {
              if (installingWorker.state === "installed") {
                if (navigator.serviceWorker.controller) {
                  config.log && console.log("Service Worker已安装更新");
                  if (config && config.onUpdate) {
                    config.onUpdate(swReg);
                  }
                } else {
                  config.log && console.log("Service Worker已安装");
  ...
  // Chromium内核和Firefox浏览器支持广播
  if (
    BrowserType.supporter === "chrome" ||
    BrowserType.supporter === "firefox"
  ) {
